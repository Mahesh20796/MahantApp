  ...
  template: `
    <div class="card animate-fade-in" style="margin-bottom: 32px;">
      <div class="page-header">
        ...
        <button *ngIf="canCreateMember" class="btn" [ngClass]="showForm ? 'btn-danger' : 'btn-primary'" (click)="toggleForm()" style="justify-content: center;">
      ...
      <div *ngIf="!showForm" style="margin-top: 32px;">
        ...
        <div class="table-responsive hide-on-mobile">
          <table class="table">
            <thead>
              <tr>
                <th *ngIf="canDeleteMember" style="width: 40px;"><input type="checkbox" [checked]="isAllSelected()" (change)="toggleAll($event)"></th>
            ...
            <tbody>
              <tr *ngFor="let m of filteredMembers" class="table-row-hover">
                <td *ngIf="canDeleteMember"><input type="checkbox" [checked]="selectedMemberIds.has(m.id!)" (change)="toggleSelection(m.id!)"></td>
                <td>
                ...
                <td style="text-align: right;">
                  ...
                  <button *ngIf="canDeleteMember" class="btn" style="padding: 8px; background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.1); color: var(--danger); font-size: 0.85rem; margin-left: 8px;" (click)="deleteMember(m.id!)">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div class="show-on-mobile mobile-card-list">
          <div *ngFor="let m of filteredMembers" class="mobile-card">
            ...
            <div style="margin-top: 16px; pt-16; border-top: 1px solid var(--border-color); display: flex; gap: 12px; padding-top: 16px;">
               ...
               <button *ngIf="canDeleteMember" class="btn btn-danger" style="flex: 1; justify-content: center;" (click)="deleteMember(m.id!)">Delete</button>
            </div>
          </div>
        </div>
...
export class MemberManagementComponent implements OnInit {
  ...
  get canViewAllMembers() {
    return this.isAdmin || this.auth.hasPermission('members', 'view');
  }

  get canCreateMember() {
    return this.isAdmin || this.auth.hasPermission('members', 'create');
  }

  get canEditMember() {
    return this.isAdmin || this.auth.hasPermission('members', 'edit');
  }

  get canDeleteMember() {
    return this.isAdmin || this.auth.hasPermission('members', 'delete');
  }
